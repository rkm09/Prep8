...
//    dp; time: O(n), space: O(n)
    public static String stoneGameIII(int[] stoneValue) {
        ...
        int diff = maxDiff(stoneValue, 0, n, memo);

        return diff > 0 ? "Alice" : (diff == 0 ? "Tie" : "Bob");
    }

    private static int maxDiff(int[] stoneValue, int i, int n, Integer[] memo) {
        if (i >= n)
            return 0;
        if (memo[i] != null)
            return memo[i];
        ...
        for (int k = 0; k < 3 && i + k < n; k++) {
            ...
            int currentDiff = currentSum - maxDiff(stoneValue, i + k + 1, n, memo);
            maxScore = Math.max(maxScore, currentDiff);
        }
