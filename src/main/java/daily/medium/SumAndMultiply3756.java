...
public class SumAndMultiply3756 {
    ...
    private static final int MAX_N = 1_000_01;
...
//    prefix array; time: O(N), space: O(N)
    public static int[] sumAndMultiply(String s, int[][] queries) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            prefX[i + 1] = digit > 0 ? (10 * prefX[i] + digit) % MOD : prefX[i];
        }
...
Return an array of integers where answer[i] is the answer to the ith query.
