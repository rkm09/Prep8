...
//        base case: only one element left, current player must take it
        if (i == j)
            return nums[i];
        if (memo[i][j] != null)
            return memo[i][j];
//        take nums[i], opponent gets remaining subarray [i + 1, j]
        int pickLeft = nums[i] - maxDiff(nums, i + 1, j, memo);
//        take nums[j], opponent gets remaining subarray [i, j - 1]
...
Hence, player 1 will never be the winner, you need to return false.
Example 2:
